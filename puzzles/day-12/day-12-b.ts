...
export async function day12b(dataPath?: string) {
  ...
  const springs: Spring[] = data.map((line) => {
    ...
    const sections = rawSections.concat(rawSections, rawSections, rawSections, rawSections).split('.').filter((c) => c !== '');
    const damaged = rawGroups.concat(rawGroups, rawGroups, rawGroups, rawGroups).split(',').map((g) => parseInt(g));
    return { sections, damaged };
  });

  console.log('Start recursion');
  const arrangements = springs.map((spring, i) => {
    const result = findSpringArrangements(spring);
    console.log(`Finished ${i + 1}/${springs.length}`);
    return result;
  });
